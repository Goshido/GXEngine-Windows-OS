...
#define WM_GX_MOUSEWHEEL			0x020A
...
GXBool GXCALL GXXInputInit ()
{
	...
	PFNGXXINPUTINITPROC GXXInputInit = (PFNGXXINPUTINITPROC)GetProcAddress ( gx_GXEngineDLLModuleHandle, "?GXXInputInit@@YGXAAUGXXInputFunctions@@@Z" );
	if ( !GXXInputInit )
	{
...
GXVoid GXInput::BindKeyFunc ( PFNGXKEYPROC callback, GXInt vk_key, EGXInputButtonState eState )
{
...
GXVoid GXInput::BindTypeFunc ( PFNGXTYPEPROC callback )
{
	symbol = 0;
	OnType = callback;
}
...
GXVoid GXInput::BindGamepadKeyFunc ( PFNGXKEYPROC callback, GXInt gamepad_key, EGXInputButtonState eState )
{
...
LRESULT CALLBACK GXInput::InputProc ( HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam )
{
	switch ( msg )
	{
		...
		case WM_SYSKEYDOWN:
		{
			...
			GXWChar buff[ 20 ];
			if ( ToUnicode ( wParam, lParam, gx_inputKeys, buff, 20, 0 ))
				OnType ( buff[ 0 ] );
		}
		return 0;
