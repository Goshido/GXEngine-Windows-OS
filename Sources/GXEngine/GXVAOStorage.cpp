...
GXVoid GXCALL GXCreateVAOFromObj ( GXVAOUnit &unit, const GXUByte* data, GXUInt size )
{
	...
	//{
		...
		const GXInt vertexOffset = 0;
		const GXInt uvOffset = sizeof ( GXVec3 );
		const GXInt normalOffset = sizeof ( GXVec3 ) + sizeof ( GXVec2 );
		const GXInt tangentOffset = sizeof ( GXVec3 ) + sizeof ( GXVec2 ) + sizeof ( GXVec3 );
		const GXInt bitangentOffset = sizeof ( GXVec3 ) + sizeof ( GXVec2 ) + sizeof ( GXVec3 ) + sizeof ( GXVec3 );
		
		const GXInt stride = sizeof ( GXVec3 ) + sizeof ( GXVec2 ) + sizeof ( GXVec3 ) + sizeof ( GXVec3 ) + sizeof ( GXVec3 );
...
GXVoid GXCALL GXCreateVAOFromNativeStaticMesh ( GXVAOUnit &out, const GXNativeStaticMeshInfo &info )
{
	...
	GXUInt vertexSize = info.numVertices * sizeof ( GXVec3 );
	GXUInt uvSize = info.numUVs * sizeof ( GXVec2 );
	GXUInt normalSize = info.numNormals * sizeof ( GXVec3 );
	GXUInt tangentBitangentSize = 2 * info.numTBPairs * sizeof ( GXVec3 );

	GXUInt size = vertexSize + uvSize + normalSize + tangentBitangentSize;
	...
	//{
		...
		const GXInt vertexOffset = 0;
		const GXInt uvOffset = ( uvSize > 0 ) ? vertexOffset + sizeof ( GXVec3 ) : vertexOffset;
		const GXInt normalOffset = ( normalSize > 0 ) ? uvOffset + sizeof ( GXVec2 ) : uvOffset;
		const GXInt tangentOffset = ( tangentBitangentSize > 0 ) ? normalOffset + sizeof ( GXVec3 ) : 0;
		const GXInt bitangentOffset = ( tangentBitangentSize > 0 ) ? tangentOffset + sizeof ( GXVec3 ) : 0;
		
		GXInt stride = sizeof ( GXVec3 );
