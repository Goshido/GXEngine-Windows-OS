...
class GXPhysicsBase : public GXPhysics
{
	...
	private:
		...
		static PxSceneQueryHitType::Enum BatchQueryPreFilterShader ( PxFilterData filterData0, PxFilterData filterData1, const void* constantBlock, PxU32 constantBlockSize, PxSceneQueryFilterFlags &filterFlags );
...
GXPhysicsBase::GXPhysicsBase ()
{
	...
	if ( !( pCooking = PxCreateCooking ( PX_PHYSICS_VERSION, *pFoundation, PxCookingParams () ) ) )
	{
	...
	if ( !( pControllerManager = PxCreateControllerManager ( *pFoundation ) ) )
	{
	...
	batchQueryDesc.userRaycastResultBuffer = queryResults;
	batchQueryDesc.userRaycastHitBuffer = queryHits;
	batchQueryDesc.raycastHitBufferSize = maxQueries;
...
PxVehicleDrive4W* GXPhysicsBase::CreateVehicle ( const GXVehicleInfo &info )
{
	...
	differentialData.mType = info.differentialType;
	...
	gearData.mNumRatios = info.numGears;
	...
	for ( GXUChar i = 0; i < info.numWheels; i++ )
	{
		vehicle->setWheelShapeMapping ( i, i );
		vehicle->setSceneQueryFilterData ( i, filterData );
	}
	...
	while ( accumulator >= GX_PHYSICS_SIMULATE_DELAY )
	{
		if ( pScene )
		{
			...
			if ( numVehicles )
			{
				...
				PxVehicleUpdates ( (GXFloat)GX_PHYSICS_SIMULATE_DELAY, gravity, *tireSurfacePairs , numVehicles, vehicles );
			}

			PxU32 nbActiveTransforms;
			PxActiveTransform* activeTransforms = pScene->getActiveTransforms ( nbActiveTransforms );
			...
			for ( GXUInt i = 0; i < numVehicles; i++ )
			{
				...
				{
					PxTransform trans = PxShapeExt::getGlobalPose ( *carShapes [ shapeID ] );
...
{
	vehicle->mDriveDynData.setAnalogInput ( value, controlType );
}
...
PxSceneQueryHitType::Enum GXPhysicsBase::BatchQueryPreFilterShader ( PxFilterData filterData0, PxFilterData filterData1, const void* constantBlock, PxU32 constantBlockSize, PxSceneQueryFilterFlags &filterFlags )
{
	...
	{
		filterFlags |= PxSceneQueryFilterFlag::eSTATIC | PxSceneQueryFilterFlag::eDYNAMIC;
