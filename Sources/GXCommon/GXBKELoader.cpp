...
GXVoid GXCALL GXLoadBKE ( const GXWChar* fileName, GXBakeInfo &out_bake_info )
{
	...
	GXUShort fileNameSize = sizeof ( GXWChar ) * ( wcslen ( (GXWChar*)( buffer + h.us_FileNameOffset ) ) + 1 );
	GXUShort cacheFileNameSize = sizeof ( GXWChar ) * ( wcslen ( (GXWChar*)( buffer + h.us_CacheFileNameOffset ) ) + 1 );
