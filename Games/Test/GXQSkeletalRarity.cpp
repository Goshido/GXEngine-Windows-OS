...
GXQSkeletalRarity::GXQSkeletalRarity ()
{
	Load3DModel ();
	InitCharacterContent ();
}

GXVoid GXQSkeletalRarity::InitCharacterContent ()
{
...
GXVoid GXQSkeletalRarity::InitUniforms ()
{
...
GXVoid GXQSkeletalRarity::Load3DModel ()
{
	GXLoadPSK ( L"../3D Models/Test/Rigged Rarity/Rarity.PSK", &skminfo );
	GXLoadMTR ( L"../Materials/Test/Rarity.mtr", mat_info );
	GXGetTexture ( mat_info.textures[ 0 ] );
	GXGetTexture ( mat_info.textures[ 1 ] );
}

GXVoid GXQSkeletalRarity::SetLocation ( GXFloat* Loc )
{
	GXMesh::SetLocation ( Loc );
}

GXVoid GXQSkeletalRarity::SetLocation ( GXFloat x, GXFloat y, GXFloat z )
{
	GXMesh::SetLocation ( x, y, z );
}

GXVoid GXQSkeletalRarity::SetRotation ( GXFloat* Rot_rad )
{
	GXMesh::SetRotation ( Rot_rad );
}

GXVoid GXQSkeletalRarity::SetRotation ( GXQuat qt )
{
	GXMesh::SetRotation ( qt );
}

GXVoid GXQSkeletalRarity::SetRotation ( GXFloat pitch_rad, GXFloat yaw_rad, GXFloat roll_rad )
{
	GXMesh::SetRotation ( pitch_rad, yaw_rad, roll_rad );
}

GXVoid GXQSkeletalRarity::Update ( GXDword deltaTime )
{
